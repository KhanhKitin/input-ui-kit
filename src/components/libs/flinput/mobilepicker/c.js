...
const InputScroller = (props) => {
 ...
 const [source, setSource] = React.useState([{text:"a"}]);
 const [sourceLength, setSourceLength] = React.useState(0);
 const [type, setType] = React.useState('infinite');
...
useEffect(() => {
   divRef.current.addEventListener('touchstart', _touchstart, { passive: true});
   // document.addEventListener('mousedown', _touchstart, { passive: true});

   divRef.current.addEventListener('touchend', _touchend, { passive: true});
   // document.addEventListener('mouseup', _touchend, { passive: true});

   return () => {
    divRef.current.removeEventListener('touchstart', _touchstart);
    // document.removeEventListener('mousedown', _touchstart);
    divRef.current.removeEventListener('touchend', _touchend);
    // document.removeEventListener('mouseup', _touchend);
  };
...
const _animateMoveByInitV = async (initV, lastScroll) => {
 ...
 const _stop = () => {
  ...
  const _touchstart = (e) => {
    props.isDrag(true)
    divRef.current.addEventListener('touchmove', _touchmove, { passive: true});
   // document.addEventListener('mousemove', _touchmove, { passive: true});
  ...
  const _touchend = (e) => {
    props.isDrag(false)
    divRef.current.removeEventListener('touchmove', _touchmove);
...
const   _normalizeScroll = (inscroll) => {
    let normalizedScroll = inscroll;
